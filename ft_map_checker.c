...
/*   Updated: 2022/01/28 11:42:54 by mgolinva         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_error_m(int ctE, int ctP, t_prg *prg)
{
	...
	if (ctP > 1)
	{
		printf("This ain't a coop game,"
			   	"only one character on the map please.\n");
		exit (0);
	}
	if (ctE == 0)
	{
		printf("We cannot get out. a shadow moves"
				" in the dark. add an exit you fool !\n");
		exit (0);
	}
	if (ctE > 1)
	{
